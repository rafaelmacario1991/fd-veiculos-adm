...
ssh root@$VPS "mkdir -p /var/www/fdveiculos"

Write-Host "[2/4] Copiando config do Nginx..." -ForegroundColor Cyan
scp deploy\fdveiculos-nginx.conf "root@${VPS}:${NGINX}"
...
ssh root@$VPS "certbot --nginx -d sistemafdveiculos.com.br -d www.sistemafdveiculos.com.br --non-interactive --agree-tos -m $EMAIL"
